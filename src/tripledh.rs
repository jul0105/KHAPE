...
pub fn compute_client(B: CurvePoint, Y: CurvePoint, a: CurveScalar, x: CurveScalar) -> [u8; 32] {
    // B^x || Y^a || Y^x
    let o_client_1 = compute_shared_key(x.to_bytes(), B);
    let o_client_2 = compute_shared_key(a.to_bytes(), Y);
    let o_client_3 = compute_shared_key(x.to_bytes(), Y);
...
pub fn compute_server(A: CurvePoint, X: CurvePoint, b: CurveScalar, y: CurveScalar) -> [u8; 32] {
    // X^b || A^y || X^y
    let o_server_1 = compute_shared_key(b.to_bytes(), X);
    let o_server_2 = compute_shared_key(y.to_bytes(), A);
    let o_server_3 = compute_shared_key(y.to_bytes(), X);
