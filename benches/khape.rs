...
pub fn client_initialization(c: &mut Criterion) {
    ...
    c.bench_function("client_initialization", |b| b.iter(|| Client::new(Parameters::new(USE_OPRF, USE_SLOW_HASH), uid)));
}
...
pub fn server_register_start(c: &mut Criterion) {
    ...
    c.bench_function("server_register_start", |b| b.iter(|| server.register_start(black_box((register_request)))));
}
...
pub fn client_register_finish(c: &mut Criterion) {
    ...
    c.bench_function("client_register_finish", |b| b.iter(|| client.register_finish(black_box(register_response), black_box(oprf_client_state))));
}
...
pub fn server_register_finish(c: &mut Criterion) {
    ...
    c.bench_function("server_register_finish", |b| b.iter(|| server.register_finish(black_box(register_finish), black_box(pre_register_secrets))));
}
